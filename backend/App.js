...
io.on("connection", (socket) => {
  ...
  socket.on("/api/Usuarios/Login",async (data) => {
    ...
    if (errors.length > 0) {
      ...
      if(emailUsuario) {
        console.log("entra a conprobacion de gmail");
        ...
        console.log("no entra a conprobacion de gmail");
  ...
  socket.on("/api/Usuarios",async (data,res) => {
    ...
    if (errors.length > 0) {
      ...
      const emailUsuario =await usuarioModel.findOne({Correo:Correo});
      if(emailUsuario) {
        console.log({message:"El gmail ya fue registrado con anterioridad",'success':false});
      } 
      else {
        console.log({message:'Ya fuistes registrado con éxito :)','success':true});
